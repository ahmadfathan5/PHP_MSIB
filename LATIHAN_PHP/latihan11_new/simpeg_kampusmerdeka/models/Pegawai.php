...
class Pegawai
{
    ...
    public function dataPegawai()
    {
        $sql = "SELECT p.*, d.nama AS bagian, j.nama AS jab
                FROM pegawai p
                INNER JOIN divisi d ON d.id = p.divisi_id1
    ...
    {
        $sql = "INSERT INTO pegawai (nip, nama, gender, foto, tmp_lahir, tgl_lahir, jabatan_id, divisi_id1, alamat)
                VALUES (?,?,?,?,?,?,?,?,?)";
    ...
    {
        $sql = "UPDATE pegawai SET nip=?, nama=?, gender=?, foto=?, tmp_lahir=?, tgl_lahir=?, jabatan_id=?, divisi_id1=?, alamat=?
                WHERE id=?";
        $td = $this->koneksi->prepare($sql);
        $td->execute($data);
    }
